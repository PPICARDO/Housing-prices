...
library(ggmap)
library(leaflet)
setwd("C:/Users/Usuario/Documents/Housing-prices-tesina/09_meli")
# comenzamos con los datos de set

# Mercadolibre set

meli_setiembre_orig <- read_csv("2018_09_24_Mercadolibre.csv", quote ="")
str(meli_setiembre_orig, list.len = ncol(meli_setiembre_orig))
dim(meli_setiembre_orig)
# 48841   209


# Tomamos solo las de Montevideo, en esta bajada está todo el país
meli_setiembre_1 <- subset(meli_setiembre_orig, state == "Montevideo" & category_1=="Venta")
meli_setiembre_1 <- data.frame(meli_setiembre_1)
head(meli_setiembre_1)
str(meli_setiembre_1, list.len = ncol(meli_setiembre_1))
dim(meli_setiembre_1)
#  35707   209

# Sacamos una primer conjunto de variables que no nos interesarán 
# Básicamente refieren a alquileres o a terrenos/campos

meli_setiembre_1$TV.por.cable <- NULL
meli_setiembre_1$Desayuno.incluido <- NULL
meli_setiembre_1$Heladera.con.freezer <- NULL
meli_setiembre_1$Hidromasaje <- NULL
meli_setiembre_1$Lavavajillas <- NULL
meli_setiembre_1$Lavarropa.Secarropa <- NULL
meli_setiembre_1$Microondas <- NULL
meli_setiembre_1$Servicio.de.limpieza <- NULL  
meli_setiembre_1$Wifi <- NULL
meli_setiembre_1$Apto.para.familias.ni.f1.os <- NULL
meli_setiembre_1$Forestaci.f3.n <- NULL
meli_setiembre_1$Silos <- NULL
meli_setiembre_1$Bebederos <- NULL
meli_setiembre_1$Corrales <- NULL
meli_setiembre_1$Corral <- NULL
meli_setiembre_1$Potreros <- NULL
meli_setiembre_1$Check.in <- NULL
meli_setiembre_1$Check.out <- NULL
meli_setiembre_1$Desde <- NULL #
meli_setiembre_1$Desde..Fecha. <- NULL
meli_setiembre_1$Hasta <- NULL #
meli_setiembre_1$Estad.ed.a.M.ed.nima <- NULL
meli_setiembre_1$Tipo.de.campo <- NULL
meli_setiembre_1$Apto.para.fumadores <- NULL
meli_setiembre_1$Hect.e1.reas.totales <- NULL
meli_setiembre_1$Distancia.al.asfalto..Km. <- NULL
meli_setiembre_1$Grupo.eletr.f3.geno <- NULL
meli_setiembre_1$Manga..ancho. <- NULL
meli_setiembre_1$Restaurante <- NULL
meli_setiembre_1$Linea.telef.f3.nica <- NULL
meli_setiembre_1$Tel.e9.fono <- NULL
meli_setiembre_1$Luz.a.motor <- NULL
meli_setiembre_1$Molinos <- NULL
meli_setiembre_1$Hasta..Fecha. <- NULL
meli_setiembre_1$N.fa.mero.de.camas <- NULL
meli_setiembre_1$Tipo.de.habitaci.f3.n <- NULL
meli_setiembre_1$Estacionamiento.de.cortes.ed.a <- NULL
meli_setiembre_1$Estacionamiento <- NULL


# aparecieron más variables en esta bajada, vamos a sacar alguna más
meli_setiembre_1$Horario.check.in <- NULL
meli_setiembre_1$Horario.check.out<- NULL
meli_setiembre_1$Estad.ed.a.m.ed.nima..noches. <- NULL
meli_setiembre_1$L.ed.nea.telef.f3.nica <- NULL
meli_setiembre_1$Business.center<- NULL
meli_setiembre_1$Cisterna<- NULL
meli_setiembre_1$Grupo.electr.f3.geno  <- NULL
meli_setiembre_1$Solo.familias <- NULL  
meli_setiembre_1$Apto.para.familias.con.ni.f1.os <- NULL
meli_setiembre_1$Vajilla <- NULL
meli_setiembre_1$Hu.e9.spedes <- NULL
meli_setiembre_1$X.c1.rea.de.juegos.infantiles   <- NULL
meli_setiembre_1$Caballeriza <- NULL
meli_setiembre_1$No <- NULL
meli_setiembre_1$Valet.parking   <- NULL
meli_setiembre_1$Valor.del.IPTU  <- NULL

meli_setiembre_1$Servicio.de.desayuno <- NULL
meli_setiembre_1$X.c1.rea.de.cine <- NULL
meli_setiembre_1$Galer.ed.a <- NULL
meli_setiembre_1$Plataformas.para.trailers <- NULL
meli_setiembre_1$TV <- NULL

meli_setiembre_1$Lavarropa <- NULL
meli_setiembre_1$Camas<- NULL
meli_setiembre_1$Estacionamiento.para.visitantes <- NULL
meli_setiembre_1$Sal.f3.n.de.fiestas <- NULL
meli_setiembre_1$Sitio.de.origen <- NULL
meli_setiembre_1$Disposici.f3.n.del.lote  <- NULL
meli_setiembre_1$B.e1.scula <- NULL
meli_setiembre_1$Probador <- NULL
meli_setiembre_1$Rampa.para.silla.de.ruedas <- NULL
meli_setiembre_1$Distancia.al.asfalto <- NULL
meli_setiembre_1$X.c1.rea.de.comedor  <- NULL
meli_setiembre_1$X.c1.reas.verdes <- NULL
meli_setiembre_1$Ba.f1.os.por.piso <- NULL
meli_setiembre_1$N.fa.mero.de.oficinas <- NULL
meli_setiembre_1$Oficinas.por.piso <- NULL
meli_setiembre_1$Superficie.cubierta.del.casco <- NULL
meli_setiembre_1$Tipo.de.bodega <- NULL
meli_setiembre_1$Heladera <- NULL
meli_setiembre_1$Caja.de.seguridad <- NULL
...
meli_setiembre_1 <- rename(meli_setiembre_1, 
...
num <- c("precio", "zip", "lat", "long", "sup_cub", "antiguedad", "sup_tot", "dormitorios", "banos", "ascensores",
         ...
         "altura", "sup_contru_2") 
# meli_setiembre_1[,num] <- lapply(meli_setiembre_1[,num], as.numeric)

# variables chr
chr <- c("titulo", "direccion")
meli_setiembre_1[,chr] <- lapply(meli_setiembre_1[,chr], as.character)
...
meli_setiembre_1[,logic] <- lapply(meli_setiembre_1[,logic], as.logical)
...
factor <- c("categ", "operacion",	 "tipo_inmueble","moneda", "condicion","barrio", "departamento",	 	
            ...
            "soporte_piso")
meli_setiembre_1[,factor] <- lapply(meli_setiembre_1[,factor], as.factor)


str(meli_setiembre_1, list.len = ncol(meli_setiembre_1))
summary(meli_setiembre_1, list.len = ncol(meli_setiembre_1))
head(meli_setiembre_1)
dim(meli_setiembre_1)
# 35707   144 variables

#genero variables para identificar momento de bajada y fuente, en este caso Mercadolibre
meli_setiembre_1$mes <- "setiembre"
meli_setiembre_1$dia <- 24
meli_setiembre_1$ano <- 2018
meli_setiembre_1$fuente <- "meli"

# defino data frame "final"
meli_2018_09 <- as.data.frame(meli_setiembre_1)
str(meli_2018_09, list.len=ncol(meli_2018_09))
dim(meli_2018_09)
# 35707 obs 148 variables

# guardo en Rdata - base ordenada 1 de datos MELI febrero
save(meli_2018_09,file="2018_09_24_meli.Rda")

# para traerlo
load("2018_09_24_meli.Rda")
