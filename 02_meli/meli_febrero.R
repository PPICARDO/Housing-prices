...
meli_feb_orig <- read_csv("C:/Users/Usuario/Desktop/PP/Tesis/Scraping/2018_02_23_mercadolibre solo/venta_2018_02_23.csv")
...
# Quedamos con 132 variables, vamos a categorizar una a una y, de paso, modificar los nombres
...
meli_feb_1$serviciodepend <- as.factor(meli_feb_1$Dependencia.de.servicio)
...
meli_feb_1$salon_comunal <- as.factor(meli_feb_1$Sal.f3.n.de.uso.com.fa.n)
...
meli_feb_1$familia <- as.factor(meli_feb_1$Apto.para.familias.ni.f1.os)
meli_feb_1$estacionamiento <- as.factor(meli_feb_1$Estacionamiento)
meli_feb_1$mascotas <- as.factor(meli_feb_1$Permite.mascotas)
meli_feb_1$serv_limpieza <- as.factor(meli_feb_1$Servicio.de.limpieza)
...
map <- get_map("Montevideo", zoom = 12, maptype = 'roadmap')
plot(map)
...
apto_casa <- ifelse(meli_feb_map$tipo_inmueble=="Apartamentos" , "Apartamentos", ifelse(meli_feb_map$tipo_inmueble=="Casas", "Casas", "Otros/Sin clasificar"))

ggmap(map) +
  geom_point(data=meli_feb_map, aes(x = long, y = lat, color=apto_casa) , size=0.5 , alpha=0.5)+
  scale_color_manual(breaks=c("Apartamentos", "Casas", "Otros/Sin clasificar"), values=c("dodgerblue","firebrick1", "gray67"))
