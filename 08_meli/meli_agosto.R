...
library(ggmap)
library(leaflet)
setwd("C:/Users/Usuario/Documents/Housing-prices-tesina/08_meli")
# comenzamos con los datos de agosto

# Mercadolibre agosto

meli_agosto_orig <- read_csv("2018_08_20_Mercadolibre.csv", quote ="")
str(meli_agosto_orig, list.len = ncol(meli_agosto_orig))
dim(meli_agosto_orig)
# 48063   207


# Tomamos solo las de Montevideo, en esta bajada está todo el país
meli_agosto_1 <- subset(meli_agosto_orig, state == "Montevideo" & category_1=="Venta")
meli_agosto_1 <- data.frame(meli_agosto_1)
head(meli_agosto_1)
str(meli_agosto_1, list.len = ncol(meli_agosto_1))
dim(meli_agosto_1)
#  35234   207
...
meli_agosto_1$TV.por.cable <- NULL
meli_agosto_1$Desayuno.incluido <- NULL
meli_agosto_1$Heladera.con.freezer <- NULL
meli_agosto_1$Hidromasaje <- NULL
meli_agosto_1$Lavavajillas <- NULL
meli_agosto_1$Lavarropa.Secarropa <- NULL
meli_agosto_1$Microondas <- NULL
meli_agosto_1$Servicio.de.limpieza <- NULL  
meli_agosto_1$Wifi <- NULL
meli_agosto_1$Apto.para.familias.ni.f1.os <- NULL
meli_agosto_1$Forestaci.f3.n <- NULL
meli_agosto_1$Silos <- NULL
meli_agosto_1$Bebederos <- NULL
meli_agosto_1$Corrales <- NULL
meli_agosto_1$Corral <- NULL
meli_agosto_1$Potreros <- NULL
meli_agosto_1$Check.in <- NULL
meli_agosto_1$Check.out <- NULL
meli_agosto_1$Desde <- NULL #
meli_agosto_1$Desde..Fecha. <- NULL
meli_agosto_1$Hasta <- NULL #
meli_agosto_1$Estad.ed.a.M.ed.nima <- NULL
meli_agosto_1$Tipo.de.campo <- NULL
meli_agosto_1$Apto.para.fumadores <- NULL
meli_agosto_1$Hect.e1.reas.totales <- NULL
meli_agosto_1$Distancia.al.asfalto..Km. <- NULL
meli_agosto_1$Grupo.eletr.f3.geno <- NULL
meli_agosto_1$Manga..ancho. <- NULL
meli_agosto_1$Restaurante <- NULL
meli_agosto_1$Linea.telef.f3.nica <- NULL
meli_agosto_1$Tel.e9.fono <- NULL
meli_agosto_1$Luz.a.motor <- NULL
meli_agosto_1$Molinos <- NULL
meli_agosto_1$Hasta..Fecha. <- NULL
meli_agosto_1$N.fa.mero.de.camas <- NULL
meli_agosto_1$Tipo.de.habitaci.f3.n <- NULL
meli_agosto_1$Estacionamiento.de.cortes.ed.a <- NULL
meli_agosto_1$Estacionamiento <- NULL


# aparecieron más variables en esta bajada, vamos a sacar alguna más
meli_agosto_1$Horario.check.in <- NULL
meli_agosto_1$Horario.check.out<- NULL
meli_agosto_1$Estad.ed.a.m.ed.nima..noches. <- NULL
meli_agosto_1$L.ed.nea.telef.f3.nica <- NULL
meli_agosto_1$Business.center<- NULL
meli_agosto_1$Cisterna<- NULL
meli_agosto_1$Grupo.electr.f3.geno  <- NULL
meli_agosto_1$Solo.familias <- NULL  
meli_agosto_1$Apto.para.familias.con.ni.f1.os <- NULL
meli_agosto_1$Vajilla <- NULL
meli_agosto_1$Hu.e9.spedes <- NULL
meli_agosto_1$X.c1.rea.de.juegos.infantiles   <- NULL
meli_agosto_1$Caballeriza <- NULL
meli_agosto_1$No <- NULL
meli_agosto_1$Valet.parking   <- NULL
meli_agosto_1$Valor.del.IPTU  <- NULL

meli_agosto_1$Servicio.de.desayuno <- NULL
meli_agosto_1$X.c1.rea.de.cine <- NULL
meli_agosto_1$Galer.ed.a <- NULL
meli_agosto_1$Plataformas.para.trailers <- NULL
meli_agosto_1$TV <- NULL

meli_agosto_1$Lavarropa <- NULL
meli_agosto_1$Camas<- NULL
meli_agosto_1$Estacionamiento.para.visitantes <- NULL
meli_agosto_1$Sal.f3.n.de.fiestas <- NULL
meli_agosto_1$Sitio.de.origen <- NULL
meli_agosto_1$Disposici.f3.n.del.lote  <- NULL
meli_agosto_1$B.e1.scula <- NULL
meli_agosto_1$Probador <- NULL
meli_agosto_1$Rampa.para.silla.de.ruedas <- NULL
meli_agosto_1$Distancia.al.asfalto <- NULL
meli_agosto_1$X.c1.rea.de.comedor  <- NULL
meli_agosto_1$X.c1.reas.verdes <- NULL
meli_agosto_1$Ba.f1.os.por.piso <- NULL
meli_agosto_1$N.fa.mero.de.oficinas <- NULL
meli_agosto_1$Oficinas.por.piso <- NULL
meli_agosto_1$Superficie.cubierta.del.casco <- NULL
meli_agosto_1$Tipo.de.bodega <- NULL
meli_agosto_1$Heladera <- NULL
meli_agosto_1$Caja.de.seguridad <- NULL
...
meli_agosto_1 <- rename(meli_agosto_1, 
...
num <- c("precio", "zip", "lat", "long", "sup_cub", "antiguedad", "sup_tot", "dormitorios", "banos", "ascensores",
         ...
         "altura", "sup_contru_2") 
# meli_agosto_1[,num] <- lapply(meli_agosto_1[,num], as.numeric)

# variables chr
chr <- c("titulo", "direccion")
meli_agosto_1[,chr] <- lapply(meli_agosto_1[,chr], as.character)
...
meli_agosto_1[,logic] <- lapply(meli_agosto_1[,logic], as.logical)
...
factor <- c("categ", "operacion",	 "tipo_inmueble","moneda", "condicion","barrio", "departamento",	 	
            ...
            "soporte_piso")
meli_agosto_1[,factor] <- lapply(meli_agosto_1[,factor], as.factor)


str(meli_agosto_1, list.len = ncol(meli_agosto_1))
summary(meli_agosto_1, list.len = ncol(meli_agosto_1))
head(meli_agosto_1)
dim(meli_agosto_1)
# 32242 145 variables

#genero variables para identificar momento de bajada y fuente, en este caso Mercadolibre
meli_agosto_1$mes <- "agosto"
meli_agosto_1$dia <- 25
meli_agosto_1$ano <- 2018
meli_agosto_1$fuente <- "meli"

# defino data frame "final"
meli_2018_08 <- as.data.frame(meli_agosto_1)
str(meli_2018_08, list.len=ncol(meli_2018_08))
dim(meli_2018_08)
# 35234 obs 148 variables

# guardo en Rdata - base ordenada 1 de datos MELI febrero
save(meli_2018_08,file="2018_08_25_meli.Rda")

# para traerlo
load("2018_08_25_meli.Rda")
